// src/components/StrategyPanel.jsx (Versión Simplificada)
...
// Recibe el "borrador" de la estrategia y la función para actualizarlo.
export function StrategyPanel({ strategy, setStrategy }) {
  ...
  // Si la estrategia aún no se ha cargado, no mostramos nada.
  // El modal se encargará de mostrar un estado de carga.
  if (!strategy) {
    return null;
  }

  // Helper para crear los sliders de rango
  const renderRangeSlider = (name, label, tooltipText) => (
    <StrategySlider name={name} label={label} tooltipText={tooltipText} value={strategy[name]} onChange={handleChange} />
  );

  return (
    <div className="p-6 space-y-8">
      {/* Tu JSX para los títulos y secciones se mantiene igual */}
      <div>
        <h3 className="text-xl font-semibold text-gray-700 mb-4 border-l-4 border-purple-500 pl-4">Importancia de Productos</h3>
        <p className="text-sm text-gray-600 mb-4 ml-5">Define qué es "importante" para ti (escala 1-10).</p>
...
// export function StrategyPanel({ strategy, setStrategy, restoreDefaults, isLoading }) {
//   const handleChange = (e) => {
//     const { name, value, type } = e.target;
//     setStrategy(prev => ({
//       ...prev,
...
//   // Si la estrategia aún no se ha cargado, mostramos un loader
//   if (isLoading || !strategy) {
//     return <div className="p-6 text-center text-gray-500">Cargando estrategia...</div>;
//   }
